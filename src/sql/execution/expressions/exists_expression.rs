...
impl ExpressionOps for ExistsExpression {
    ...
    fn evaluate_join(&self, left_tuple: &Tuple, left_schema: &Schema, right_tuple: &Tuple, right_schema: &Schema) -> Result<Value, ExpressionError> {
        let exists = self.subquery.evaluate_join(left_tuple, left_schema, right_tuple, right_schema)?;
        let result = if self.negated { exists.is_null() } else { !exists.is_null() };
        Ok(Value::new(result))
    }
...
mod tests {
    ...
    fn test_exists_expression_evaluate() {
        ...
        let non_null_subquery = Arc::new(Expression::Constant(
        ...
        let null_subquery = Arc::new(Expression::Constant(
            ConstantExpression::new(
                Value::new(Val::Null),
    ...
    fn test_exists_expression_evaluate_join() {
        ...
        let result = exists_expr.evaluate_join(
    ...
    fn test_exists_expression_display() {
        let subquery = Arc::new(Expression::Constant(
            ConstantExpression::new(
                Value::new(1),
                Column::new("const", TypeId::Integer),
                vec![],
    ...
    fn test_exists_expression_validate() {
        ...
        let invalid_expr = ExistsExpression::new(
            subquery,
            false,
            Column::new("exists", TypeId::Integer),
        );
    ...
    fn test_exists_expression_clone_with_children() {
        let original_subquery = Arc::new(Expression::Constant(
            ConstantExpression::new(
                Value::new(1),
                Column::new("const", TypeId::Integer),
                vec![],
        ...
        let new_subquery = Arc::new(Expression::Constant(
            ConstantExpression::new(
                Value::new(2),
                Column::new("const", TypeId::Integer),
                vec![],
