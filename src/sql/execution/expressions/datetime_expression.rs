...
//     }
//
//     fn evaluate_date_trunc(&self, part: &DateTimeField, timestamp: DateTime<Utc>) -> Result<DateTime<Utc>, ExpressionError> {
...
//             }
//             DateTimeField::Month => {
//                 let date = naive.date().with_day(1)
//                     .ok_or_else(|| ExpressionError::InvalidOperation("Invalid date".to_string()))?;
...
//     }
//
//     fn evaluate_date_part(&self, part: &DateTimeField, timestamp: DateTime<Utc>) -> Result<i32, ExpressionError> {
//         match part {
...
//             DateTimeField::DayOfWeek | DateTimeField::Dow => Ok(timestamp.weekday().num_days_from_monday() as i32),
...
//         if self.args.len() != 3 {
//             return Err(ExpressionError::InvalidOperation("make_date requires year, month, and day arguments".to_string()));
//         }
//
...
//
//         let date = Utc.with_ymd_and_hms(year, month as u32, day as u32, 0, 0, 0)
//             .earliest()
//             .ok_or_else(|| ExpressionError::InvalidOperation("Invalid date components".to_string()))?;
//
//         Ok(Value::new(date.to_rfc3339()))
//     }
//
//     fn evaluate_make_time(&self, tuple: &Tuple, schema: &Schema) -> Result<Value, ExpressionError> {
//         if self.args.len() != 3 {
//             return Err(ExpressionError::InvalidOperation("make_time requires hour, minute, and second arguments".to_string()));
//         }
//
...
//
//         let time = Utc.with_ymd_and_hms(0, 0, 0, hour as u32, min as u32, sec as u32)
//             .earliest()
//             .ok_or_else(|| ExpressionError::InvalidOperation("Invalid time components".to_string()))?;
//
//         Ok(Value::new(time.to_rfc3339()))
//     }
//
//     fn evaluate_make_timestamp(&self, tuple: &Tuple, schema: &Schema) -> Result<Value, ExpressionError> {
//         if self.args.len() != 6 {
...
//
//         let timestamp = Utc.with_ymd_and_hms(year, month as u32, day as u32, hour as u32, min as u32, sec as u32)
//             .earliest()
//             .ok_or_else(|| ExpressionError::InvalidOperation("Invalid timestamp components".to_string()))?;
//
//         Ok(Value::new(timestamp.to_rfc3339()))
//     }
// }
//
...
//                     if self.args.len() != 1 {
//                         return Err(ExpressionError::InvalidOperation("date_trunc requires exactly one argument".to_string()));
//                     }
//
...
//                                 .single()
//                                 .ok_or_else(|| ExpressionError::InvalidOperation("Invalid timestamp value".to_string()))?
//                         },
//                         _ => return Err(ExpressionError::InvalidOperation("Expected timestamp value".to_string())),
//                     };
...
//                 if let Some(part) = &self.part {
//                     if self.args.len() != 1 {
//                         return Err(ExpressionError::InvalidOperation("date_part requires exactly one argument".to_string()));
//                     }
//
//                     let timestamp_val = self.args[0].evaluate(tuple, schema)?;
//                     let timestamp = match timestamp_val.get_val() {
...
//                 self.evaluate_make_date(tuple, schema)
//             },
//             DateTimeOperation::MakeTime => {
//                 self.evaluate_make_time(tuple, schema)
//             },
//             DateTimeOperation::MakeTimestamp => {
//                 self.evaluate_make_timestamp(tuple, schema)
//             },
//             DateTimeOperation::Add | DateTimeOperation::Subtract => {
//                 if self.args.len() != 2 {
//                     return Err(ExpressionError::InvalidOperation(
//                         format!("{:?} operation requires exactly two arguments", self.operation)
//                     ));
//                 }
//
...
//                     (Val::Timestamp(ts1) | Val::VarLen(ts1) | Val::ConstLen(ts1),
//                         Val::Timestamp(ts2) | Val::VarLen(ts2) | Val::ConstLen(ts2))
...
//                 if self.args.len() != 1 {
//                     return Err(ExpressionError::InvalidOperation(
//                         format!("{:?} requires exactly one argument", self.operation)
//                     ));
//                 }
//                 if self.part.is_none() {
//                     return Err(ExpressionError::InvalidOperation(
//                         format!("Missing date part for {:?}", self.operation)
//                     ));
//                 }
//
//                 // Check that the argument is a timestamp or can be converted to one
//                 let arg_type = self.args[0].get_return_type();
//                 if arg_type.get_type() != TypeId::Timestamp && arg_type.get_type() != TypeId::VarChar {
//                     return Err(ExpressionError::InvalidOperation(
//                         format!("Expected timestamp or string argument for {:?}, got {:?}",
//                                 self.operation, arg_type.get_type())
//                     ));
//                 }
//             },
//             DateTimeOperation::MakeDate => {
...
//                         return Err(ExpressionError::InvalidOperation(
//                             format!("Expected integer for argument {} of make_date, got {:?}",
//                                     i + 1, arg_type.get_type())
//                         ));
//                     }
//                 }
//             },
...
//                     if arg_type.get_type() != TypeId::Integer {
//                         return Err(ExpressionError::InvalidOperation(
//                             format!("Expected integer for argument {} of make_time, got {:?}",
//                                     i + 1, arg_type.get_type())
//                         ));
//                     }
//                 }
...
//                     if arg_type.get_type() != TypeId::Integer {
//                         return Err(ExpressionError::InvalidOperation(
//                             format!("Expected integer for argument {} of make_timestamp, got {:?}",
//                                     i + 1, arg_type.get_type())
//                         ));
//                     }
//                 }
...
//                         let valid = (left_type.get_type() == TypeId::Timestamp && right_type.get_type() == TypeId::Interval) ||
//                             (left_type.get_type() == TypeId::Interval && right_type.get_type() == TypeId::Timestamp);
//
//                         if !valid {
//                             return Err(ExpressionError::InvalidOperation(
...
//                         let valid = (left_type.get_type() == TypeId::Timestamp && right_type.get_type() == TypeId::Interval) ||
//                             (left_type.get_type() == TypeId::Timestamp && right_type.get_type() == TypeId::Timestamp);
//
//                         if !valid {
//                             return Err(ExpressionError::InvalidOperation(
...
//         // Add a timestamp value (2024-01-01 12:30:45)
//         tuple.add_value(Value::new("2024-01-01T12:30:45Z"));
//         // Add an integer value
//         tuple.add_value(Value::new(42));
//         tuple
//     }
//
//     #[test]
...
//         assert_eq!(result.get_val(), &Val::VarLen("2024-01-01T12:00:00Z".to_string()));
//     }
//
//     #[test]
//     fn test_date_part() {
...
//             DateTimeOperation::MakeDate,
//             vec![
//                 Arc::new(Expression::Constant(Value::new(2024))),
//                 Arc::new(Expression::Constant(Value::new(1))),
//                 Arc::new(Expression::Constant(Value::new(15))),
//             ],
//             None,
//             Column::new("result", TypeId::VarChar),
//         );
...
//         assert_eq!(result.get_val(), &Val::VarLen("2024-01-15T00:00:00Z".to_string()));
//     }
//
//     #[test]
...
//             DateTimeOperation::MakeTimestamp,
//             vec![
//                 Arc::new(Expression::Constant(Value::new(2024))), // year
//                 Arc::new(Expression::Constant(Value::new(1))),    // month
//                 Arc::new(Expression::Constant(Value::new(15))),   // day
//                 Arc::new(Expression::Constant(Value::new(14))),   // hour
//                 Arc::new(Expression::Constant(Value::new(30))),   // minute
//                 Arc::new(Expression::Constant(Value::new(45))),   // second
//             ],
//             None,
//             Column::new("result", TypeId::VarChar, false),
//         );
//
//         let result = make_timestamp_expr.evaluate(&tuple, &schema).unwrap();
//         assert_eq!(result.get_val(), &Val::VarLen("2024-01-15T14:30:45Z".to_string()));
//     }
//
//     #[test]
...
//             DateTimeOperation::MakeDate,
//             vec![Arc::new(Expression::Constant(Value::new(2024)))],
//             None,
//             Column::new("result", TypeId::VarChar),
//         );
...
//         assert_eq!(DateTimeField::Week(Some("monday".to_string())).to_string(), "week(monday)");
//         assert_eq!(DateTimeField::Custom("test".to_string()).to_string(), "test");
//     }
//
//     #[test]
...
//             DateTimeOperation::DateTrunc,
//             vec![Arc::new(Expression::ColumnRef(0))],
//             Some(DateTimeField::Year),
//             Column::new("result", TypeId::VarChar),
//         );
//         assert_eq!(date_trunc_expr.to_string(), "date_trunc('Year', #0)");
//
//         let make_date_expr = DateTimeExpression::new(
//             DateTimeOperation::MakeDate,
//             vec![
...
//         ]);
//
//         let mut left_tuple = Tuple::new(vec![]);
//         left_tuple.add_value(Value::new("2024-01-01T12:30:45Z"));
//
//         let mut right_tuple = Tuple::new(vec![]);
//         right_tuple.add_value(Value::new("2024-02-15T14:20:30Z"));
//
...
//                 0, 0, Column::new("left_ts", TypeId::Timestamp), vec![]
//             )))],
//             Some(DateTimeField::Month),
//             Column::new("result", TypeId::VarChar),
//         );
//
//         let result = left_trunc.evaluate_join(&left_tuple, &left_schema, &right_tuple, &right_schema).unwrap();
//         assert_eq!(result.get_val(), &Val::VarLen("2024-01-01T00:00:00Z".to_string()));
//
...
//             DateTimeOperation::DateTrunc,
//             vec![Arc::new(Expression::ColumnRef(ColumnRefExpression::new(
//                 0, 0, Column::new("right_ts", TypeId::Timestamp), vec![]
//             )))],
//             Some(DateTimeField::Month),
//             Column::new("result", TypeId::VarChar),
//         );
//
//         let result = right_trunc.evaluate_join(&left_tuple, &left_schema, &right_tuple, &right_schema).unwrap();
//         assert_eq!(result.get_val(), &Val::VarLen("2024-02-01T00:00:00Z".to_string()));
//     }
//
//     #[test]
...
//             DateTimeOperation::Add,
//             vec![
//                 Arc::new(Expression::Constant(Value::new("2024-01-01T00:00:00Z"))),
//                 Arc::new(Expression::Constant(Value::new_interval(Duration::days(1)))),
//             ],
//             None,
//             Column::new("result", TypeId::VarChar),
//         );
...
//         assert_eq!(result.get_val(), &Val::VarLen("2024-01-02T00:00:00Z".to_string()));
//
//         // Test timestamp - interval
...
//         assert_eq!(result.get_val(), &Val::VarLen("2023-12-31T00:00:00Z".to_string()));
//     }
// }
//
