...
impl DateTimeExpression {
    ...
    fn parse_iso8601_duration(duration_str: &str) -> Result<chrono::Duration, String> {
        ...
        if let Some(captures) = re.captures(duration_str) {
            let years = captures.get(1).and_then(|m| m.as_str().parse::<i64>().ok()).unwrap_or(0);
            let months = captures.get(2).and_then(|m| m.as_str().parse::<i64>().ok()).unwrap_or(0);
            let days = captures.get(3).and_then(|m| m.as_str().parse::<i64>().ok()).unwrap_or(0);
            let hours = captures.get(4).and_then(|m| m.as_str().parse::<i64>().ok()).unwrap_or(0);
            let minutes = captures.get(5).and_then(|m| m.as_str().parse::<i64>().ok()).unwrap_or(0);
            let seconds = captures.get(6).and_then(|m| m.as_str().parse::<i64>().ok()).unwrap_or(0);
            ...
            Err(format!("Invalid ISO 8601 duration format: {}", duration_str))
        }
    }
            ...
            DateTimeOperation::Add | DateTimeOperation::Subtract => {
                ...
                match (left.get_val(), right.get_val()) {
                    ...
                    if self.operation == DateTimeOperation::Add => {
                        ...
                        let duration = Self::parse_iso8601_duration(interval_str)
                            .map_err(|e| ExpressionError::InvalidOperation(format!("Invalid interval format: {}", e)))?;
        ...
        match self.operation {
            DateTimeOperation::DateTrunc => {
                if let Some(part) = &self.part {
                    ...
                    let timestamp_val = if let Expression::ColumnRef(col_ref) = self.args[0].as_ref() {
            ...
            DateTimeOperation::Add | DateTimeOperation::Subtract => {
                ...
                let mut values = dummy_tuple.get_values_mut();
                values.push(left);
                values.push(right);
            ...
            DateTimeOperation::Add | DateTimeOperation::Subtract => {
                ...
                match self.operation {
                    DateTimeOperation::Add => {
                        let valid = (left_type.get_type() == TypeId::Timestamp && right_type.get_type() == TypeId::VarChar) ||
                            (left_type.get_type() == TypeId::VarChar && right_type.get_type() == TypeId::Timestamp);
...
mod tests {
    ...
    fn create_test_tuple() -> Tuple {
        ...
        let mut values = tuple.get_values_mut();
    ...
    fn test_datetime_join_evaluation() {
        ...
        left_tuple.get_values_mut().push(Value::new("2024-01-01T12:30:45Z"));

        let mut right_tuple = Tuple::new(&[], Schema::new(vec![]), RID::new(0, 0));
        right_tuple.get_values_mut().push(Value::new("2024-02-15T14:20:30Z"));
