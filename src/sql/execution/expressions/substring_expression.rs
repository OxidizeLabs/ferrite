...
impl ExpressionOps for SubstringExpression {
    fn evaluate(&self, tuple: &Tuple, schema: &Schema) -> Result<Value, ExpressionError> {
        ...
        let string = match string_val.get_val() {
            ...
            _ => return Err(ExpressionError::InvalidOperation(
                format!("SUBSTRING requires string input, got {:?}", string_val.get_type_id())
            )),
        };
        ...
        let start_pos = match from_val.get_val() {
            ...
            _ => return Err(ExpressionError::InvalidOperation(
                format!("SUBSTRING FROM requires numeric input, got {:?}", from_val.get_type_id())
            )),
        };
        ...
        if let Some(for_expr) = &self.for_expr {
            ...
            let length = match for_val.get_val() {
                ...
                Val::TinyInt(i) => *i as i32,
                _ => return Err(ExpressionError::InvalidOperation(
                    format!("SUBSTRING FOR requires numeric input, got {:?}", for_val.get_type_id())
                )),
            };
    ...
    fn evaluate_join(
        ...
        // Evaluate the string expression
        let string_val = self.string_expr.evaluate_join(left_tuple, left_schema, right_tuple, right_schema)?;
        if string_val.is_null() {
            return Ok(Value::new(Val::Null));
        }
        ...
        // Evaluate the FROM expression (start position)
        let from_val = self.from_expr.evaluate_join(left_tuple, left_schema, right_tuple, right_schema)?;
        if from_val.is_null() {
            return Ok(Value::new(Val::Null));
        }
        ...
        // If FOR expression is provided, evaluate it
        if let Some(for_expr) = &self.for_expr {
            let for_val = for_expr.evaluate_join(left_tuple, left_schema, right_tuple, right_schema)?;
            if for_val.is_null() {
                return Ok(Value::new(Val::Null));
            }
    ...
    fn validate(&self, schema: &Schema) -> Result<(), ExpressionError> {
        ...
        if let Some(for_expr) = &self.for_expr {
            ...
            if !matches!(for_type, 
                ...
                return Err(ExpressionError::InvalidOperation(
                    format!("SUBSTRING FOR requires numeric input, got {:?}", for_type)
                ));
            }
        }
...
mod tests {
    ...
    fn test_basic_substring() {
        ...
        let substring_expr = SubstringExpression::new(
            string_expr,
            from_expr,
            Some(for_expr),
        );
