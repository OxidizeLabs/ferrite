...
pub fn record_insert_performance(row_count: usize, duration: Duration, is_bulk: bool) {
    PERFORMANCE_METRICS.total_inserts.fetch_add(row_count, Ordering::Relaxed);
    if is_bulk {
        PERFORMANCE_METRICS.bulk_inserts.fetch_add(row_count, Ordering::Relaxed);
    }
    
    let duration_ns = duration.as_nanos() as u64;
    PERFORMANCE_METRICS.insert_latency_ns.store(duration_ns, Ordering::Relaxed);
    
    if duration.as_secs() > 0 {
        let throughput = (row_count as u64) / duration.as_secs();
        PERFORMANCE_METRICS.insert_throughput_rows_per_sec.store(throughput, Ordering::Relaxed);
    }
}
...
pub fn record_buffer_pool_operation(hit: bool, pin_duration: Duration) {
    if hit {
        PERFORMANCE_METRICS.buffer_hits.fetch_add(1, Ordering::Relaxed);
    } else {
        PERFORMANCE_METRICS.buffer_misses.fetch_add(1, Ordering::Relaxed);
    }
    
    PERFORMANCE_METRICS.page_pin_duration_ns.store(
        pin_duration.as_nanos() as u64, 
        Ordering::Relaxed
    );
}
...
pub fn record_disk_io(is_write: bool, latency: Duration, is_sequential: bool) {
    if is_write {
        PERFORMANCE_METRICS.disk_writes.fetch_add(1, Ordering::Relaxed);
        PERFORMANCE_METRICS.disk_write_latency_ns.store(
            latency.as_nanos() as u64, 
            Ordering::Relaxed
        );
    } else {
        PERFORMANCE_METRICS.disk_reads.fetch_add(1, Ordering::Relaxed);
        PERFORMANCE_METRICS.disk_read_latency_ns.store(
            latency.as_nanos() as u64, 
            Ordering::Relaxed
        );
        
        if is_sequential {
            PERFORMANCE_METRICS.sequential_reads.fetch_add(1, Ordering::Relaxed);
        } else {
            PERFORMANCE_METRICS.random_reads.fetch_add(1, Ordering::Relaxed);
        }
    }
}
...
pub fn record_transaction_operation(operation: &str, duration: Duration) {
    match operation {
        "begin" => PERFORMANCE_METRICS.transactions_started.fetch_add(1, Ordering::Relaxed),
        "commit" => PERFORMANCE_METRICS.transactions_committed.fetch_add(1, Ordering::Relaxed),
        "abort" => PERFORMANCE_METRICS.transactions_aborted.fetch_add(1, Ordering::Relaxed),
        _ => 0,
    };
    
    PERFORMANCE_METRICS.transaction_duration_ns.store(
...
pub fn record_lock_operation(granted_immediately: bool, wait_time: Duration, conflict: bool) {
    PERFORMANCE_METRICS.lock_requests.fetch_add(1, Ordering::Relaxed);
    
    if granted_immediately {
        PERFORMANCE_METRICS.lock_grants_immediate.fetch_add(1, Ordering::Relaxed);
    } else {
        PERFORMANCE_METRICS.lock_waits.fetch_add(1, Ordering::Relaxed);
        PERFORMANCE_METRICS.lock_wait_time_ns.store(
    ...
    if conflict {
        PERFORMANCE_METRICS.lock_conflicts.fetch_add(1, Ordering::Relaxed);
    }
}
...
pub fn record_mvcc_operation(operation: &str, chain_length: usize, duration: Duration) {
    match operation {
        "version_traversal" => {
            PERFORMANCE_METRICS.version_chain_traversals.fetch_add(1, Ordering::Relaxed);
            PERFORMANCE_METRICS.avg_version_chain_length.store(chain_length, Ordering::Relaxed);
        },
        "visibility_check" => {
            PERFORMANCE_METRICS.tuple_visibility_checks.fetch_add(1, Ordering::Relaxed);
        },
        "snapshot_creation" => {
            PERFORMANCE_METRICS.snapshot_creation_time_ns.store(
                duration.as_nanos() as u64, 
                Ordering::Relaxed
            );
        },
...
pub fn record_query_execution(operation: &str, rows_scanned: usize, rows_returned: usize, duration: Duration) {
    match operation {
        "select" => PERFORMANCE_METRICS.select_operations.fetch_add(1, Ordering::Relaxed),
        "update" => PERFORMANCE_METRICS.update_operations.fetch_add(1, Ordering::Relaxed),
        "delete" => PERFORMANCE_METRICS.delete_operations.fetch_add(1, Ordering::Relaxed),
        "join" => PERFORMANCE_METRICS.join_operations.fetch_add(1, Ordering::Relaxed),
        "aggregation" => PERFORMANCE_METRICS.aggregation_operations.fetch_add(1, Ordering::Relaxed),
        _ => 0,
    };
    
    PERFORMANCE_METRICS.rows_scanned.fetch_add(rows_scanned, Ordering::Relaxed);
    PERFORMANCE_METRICS.rows_returned.fetch_add(rows_returned, Ordering::Relaxed);
    PERFORMANCE_METRICS.query_execution_time_ns.store(
        duration.as_nanos() as u64, 
        Ordering::Relaxed
    );
}
...
pub fn analyze_disk_io_performance() -> f64 {
    let read_latency = PERFORMANCE_METRICS.disk_read_latency_ns.load(Ordering::Relaxed);
    let write_latency = PERFORMANCE_METRICS.disk_write_latency_ns.load(Ordering::Relaxed);
    ...
    let latency_score = if avg_latency < 1_000_000 { // < 1ms
        1.0
    } else if avg_latency < 5_000_000 { // < 5ms
        0.8
...
pub fn analyze_lock_manager_performance() -> f64 {
    ...
    let immediate_grants = PERFORMANCE_METRICS.lock_grants_immediate.load(Ordering::Relaxed);
    let conflicts = PERFORMANCE_METRICS.lock_conflicts.load(Ordering::Relaxed);
    let wait_time = PERFORMANCE_METRICS.lock_wait_time_ns.load(Ordering::Relaxed);
...
pub fn analyze_mvcc_performance() -> f64 {
    let chain_length = PERFORMANCE_METRICS.avg_version_chain_length.load(Ordering::Relaxed);
    let traversals = PERFORMANCE_METRICS.version_chain_traversals.load(Ordering::Relaxed);
    let visibility_checks = PERFORMANCE_METRICS.tuple_visibility_checks.load(Ordering::Relaxed);
...
pub fn analyze_bottlenecks() -> BottleneckAnalysis {
    ...
    let worst = scores.iter().min_by(|a, b| a.1.partial_cmp(&b.1).unwrap()).unwrap();
    ...
    match worst.0 {
        "Buffer Pool" => {
            ...
            affected_operations.extend(vec!["INSERT".to_string(), "SELECT".to_string(), "UPDATE".to_string()]);
        },
        "Disk I/O" => {
        ...
        "MVCC" => {
            ...
            affected_operations.extend(vec!["READ operations".to_string(), "Long transactions".to_string()]);
        },
        _ => {}
    }
    
    let mut metrics_snapshot = HashMap::new();
    metrics_snapshot.insert("buffer_hit_ratio".to_string(), 
        ((PERFORMANCE_METRICS.buffer_hits.load(Ordering::Relaxed) as f64 / 
          (PERFORMANCE_METRICS.buffer_hits.load(Ordering::Relaxed) + 
           PERFORMANCE_METRICS.buffer_misses.load(Ordering::Relaxed)) as f64) * 100.0) as u64);
    metrics_snapshot.insert("avg_disk_latency_ms".to_string(), 
        PERFORMANCE_METRICS.disk_read_latency_ns.load(Ordering::Relaxed) / 1_000_000);
    metrics_snapshot.insert("lock_conflicts".to_string(), 
        PERFORMANCE_METRICS.lock_conflicts.load(Ordering::Relaxed) as u64);
    metrics_snapshot.insert("avg_version_chain_length".to_string(), 
        PERFORMANCE_METRICS.avg_version_chain_length.load(Ordering::Relaxed) as u64);
...
pub fn print_performance_summary() {
    ...
    let bulk_ratio = if total_inserts > 0 {
        (PERFORMANCE_METRICS.bulk_inserts.load(Ordering::Relaxed) as f64 / total_inserts as f64) * 100.0
    } else { 0.0 };
    
    println!("  Insert Performance:");
    println!("    Total Inserts: {}", total_inserts);
    println!("    Bulk Insert Ratio: {:.1}%", bulk_ratio);
    println!("    Current Throughput: {} rows/sec", 
        PERFORMANCE_METRICS.insert_throughput_rows_per_sec.load(Ordering::Relaxed));
    ...
    println!("  Page Evictions: {}", PERFORMANCE_METRICS.page_evictions.load(Ordering::Relaxed));
    println!("  Dirty Flushes: {}", PERFORMANCE_METRICS.dirty_page_flushes.load(Ordering::Relaxed));
    println!("  Performance Score: {:.2}/1.0", analyze_buffer_pool_performance());
    ...
    // === DISK I/O ANALYSIS ===
    let read_latency_ms = PERFORMANCE_METRICS.disk_read_latency_ns.load(Ordering::Relaxed) as f64 / 1_000_000.0;
    let write_latency_ms = PERFORMANCE_METRICS.disk_write_latency_ns.load(Ordering::Relaxed) as f64 / 1_000_000.0;
    ...
    println!("  Read Latency: {:.2}ms, Write Latency: {:.2}ms", read_latency_ms, write_latency_ms);
    println!("  Sequential vs Random Reads: {} vs {}", 
        PERFORMANCE_METRICS.sequential_reads.load(Ordering::Relaxed),
        PERFORMANCE_METRICS.random_reads.load(Ordering::Relaxed));
    println!("  Performance Score: {:.2}/1.0", analyze_disk_io_performance());
    ...
    println!("  Immediate Grants: {} ({:.1}%)", immediate_grants, 
        if lock_requests > 0 { (immediate_grants as f64 / lock_requests as f64) * 100.0 } else { 0.0 });
    println!("  Lock Conflicts: {} ({:.1}%)", conflicts,
        if lock_requests > 0 { (conflicts as f64 / lock_requests as f64) * 100.0 } else { 0.0 });
    println!("  Lock Manager Score: {:.2}/1.0", analyze_lock_manager_performance());
    
    // === MVCC ANALYSIS ===
    println!("🔄 MVCC ANALYSIS:");
    println!("  Version Chain Length: {}", 
        PERFORMANCE_METRICS.avg_version_chain_length.load(Ordering::Relaxed));
    println!("  Version Traversals: {}", 
        PERFORMANCE_METRICS.version_chain_traversals.load(Ordering::Relaxed));
    println!("  Visibility Checks: {}", 
        PERFORMANCE_METRICS.tuple_visibility_checks.load(Ordering::Relaxed));
    println!("  MVCC Score: {:.2}/1.0", analyze_mvcc_performance());
    
    // === TRANSACTION ANALYSIS ===
    let total_txns = PERFORMANCE_METRICS.transactions_started.load(Ordering::Relaxed);
    let committed = PERFORMANCE_METRICS.transactions_committed.load(Ordering::Relaxed);
    let aborted = PERFORMANCE_METRICS.transactions_aborted.load(Ordering::Relaxed);
    
    println!("📋 TRANSACTION ANALYSIS:");
    println!("  Started: {}, Committed: {}, Aborted: {}", total_txns, committed, aborted);
    if total_txns > 0 {
        println!("  Success Rate: {:.1}%", (committed as f64 / total_txns as f64) * 100.0);
    }
    println!("  Active Transactions: {}", PERFORMANCE_METRICS.active_transactions.load(Ordering::Relaxed));
    
    // === BOTTLENECK IDENTIFICATION ===
    println!("🚨 BOTTLENECK ANALYSIS:");
    println!("  Primary Bottleneck: {}", bottleneck_analysis.primary_bottleneck);
    println!("  Severity: {:.1}% impact", bottleneck_analysis.bottleneck_severity * 100.0);
    println!("  Affected Operations: {}", bottleneck_analysis.affected_operations.join(", "));
    ...
    println!("  Buffer Pool: {}", get_performance_grade(analyze_buffer_pool_performance()));
    println!("  Disk I/O: {}", get_performance_grade(analyze_disk_io_performance()));
    println!("  Lock Manager: {}", get_performance_grade(analyze_lock_manager_performance()));
    println!("  MVCC: {}", get_performance_grade(analyze_mvcc_performance()));
    
    // === QUERY EXECUTION ANALYSIS ===
    let total_ops = PERFORMANCE_METRICS.select_operations.load(Ordering::Relaxed) +
                    PERFORMANCE_METRICS.update_operations.load(Ordering::Relaxed) +
                    PERFORMANCE_METRICS.delete_operations.load(Ordering::Relaxed);
    
    if total_ops > 0 {
        println!("📈 QUERY EXECUTION ANALYSIS:");
        println!("  Total Operations: {}", total_ops);
        println!("  Selects: {}, Updates: {}, Deletes: {}", 
            PERFORMANCE_METRICS.select_operations.load(Ordering::Relaxed),
            PERFORMANCE_METRICS.update_operations.load(Ordering::Relaxed),
            PERFORMANCE_METRICS.delete_operations.load(Ordering::Relaxed));
        ...
        if rows_scanned > 0 {
            ...
            println!("  Query Selectivity: {:.1}% ({} returned / {} scanned)", 
        ...
        let avg_exec_time_ms = PERFORMANCE_METRICS.query_execution_time_ns.load(Ordering::Relaxed) as f64 / 1_000_000.0;
    ...
    let overall_score = (analyze_buffer_pool_performance() + 
                        ...
                        analyze_mvcc_performance()) / 4.0;
    
    println!("🏥 OVERALL SYSTEM HEALTH: {} ({:.2}/1.0)", 
        get_performance_grade(overall_score), overall_score);
...
pub fn print_real_time_alerts() {
    ...
    if buffer_hits + buffer_misses > 100 {
        ...
        if hit_ratio < 80.0 {
            alerts.push(format!("🔴 CRITICAL: Buffer hit ratio only {:.1}% (target: >95%)", hit_ratio));
        } else if hit_ratio < 95.0 {
            alerts.push(format!("🟡 WARNING: Buffer hit ratio {:.1}% (target: >95%)", hit_ratio));
        }
    }
    ...
    if read_latency_ms > 20.0 {
        alerts.push(format!("🔴 CRITICAL: Disk read latency {:.1}ms (target: <5ms)", read_latency_ms));
    } else if read_latency_ms > 10.0 {
        alerts.push(format!("🟡 WARNING: Disk read latency {:.1}ms (target: <5ms)", read_latency_ms));
    }
    
    if write_latency_ms > 20.0 {
        alerts.push(format!("🔴 CRITICAL: Disk write latency {:.1}ms (target: <5ms)", write_latency_ms));
    } else if write_latency_ms > 10.0 {
        alerts.push(format!("🟡 WARNING: Disk write latency {:.1}ms (target: <5ms)", write_latency_ms));
    }
    ...
    if lock_requests > 0 {
        ...
        if conflict_rate > 20.0 {
            alerts.push(format!("🔴 CRITICAL: Lock conflict rate {:.1}% (target: <5%)", conflict_rate));
        } else if conflict_rate > 10.0 {
            alerts.push(format!("🟡 WARNING: Lock conflict rate {:.1}% (target: <5%)", conflict_rate));
        }
    }
    ...
    let chain_length = PERFORMANCE_METRICS.avg_version_chain_length.load(Ordering::Relaxed);
    if chain_length > 50 {
        alerts.push(format!("🔴 CRITICAL: Version chain length {} (target: <10)", chain_length));
    } else if chain_length > 20 {
        alerts.push(format!("🟡 WARNING: Version chain length {} (target: <10)", chain_length));
    }
    ...
    if total_txns > 0 {
        ...
        if abort_rate > 20.0 {
            alerts.push(format!("🔴 CRITICAL: Transaction abort rate {:.1}% (target: <5%)", abort_rate));
        } else if abort_rate > 10.0 {
            alerts.push(format!("🟡 WARNING: Transaction abort rate {:.1}% (target: <5%)", abort_rate));
        }
    }
    
    // Throughput alerts
    let throughput = PERFORMANCE_METRICS.insert_throughput_rows_per_sec.load(Ordering::Relaxed);
    if throughput > 0 && throughput < 1000 {
        alerts.push(format!("🟡 WARNING: Low insert throughput {} rows/sec", throughput));
    }
...
pub fn get_performance_summary_json() -> String {
  ...
  "recommendations": {:?}
        ...
        hit_ratio,
        PERFORMANCE_METRICS.disk_read_latency_ns.load(Ordering::Relaxed) as f64 / 1_000_000.0,
        PERFORMANCE_METRICS.disk_write_latency_ns.load(Ordering::Relaxed) as f64 / 1_000_000.0,
        if PERFORMANCE_METRICS.lock_requests.load(Ordering::Relaxed) > 0 {
            (PERFORMANCE_METRICS.lock_conflicts.load(Ordering::Relaxed) as f64 / 
             PERFORMANCE_METRICS.lock_requests.load(Ordering::Relaxed) as f64) * 100.0
        } else { 0.0 },
        PERFORMANCE_METRICS.avg_version_chain_length.load(Ordering::Relaxed),
        if PERFORMANCE_METRICS.transactions_started.load(Ordering::Relaxed) > 0 {
            (PERFORMANCE_METRICS.transactions_committed.load(Ordering::Relaxed) as f64 / 
             PERFORMANCE_METRICS.transactions_started.load(Ordering::Relaxed) as f64) * 100.0
        } else { 0.0 },
        PERFORMANCE_METRICS.insert_throughput_rows_per_sec.load(Ordering::Relaxed),
...
pub fn record_system_resources(cpu_usage: f64, memory_usage_mb: u64, disk_usage_percent: f64) {
    ...
    debug!("System Resources: CPU: {:.1}%, Memory: {}MB, Disk: {:.1}%", 
    ...
    if memory_usage_mb > 8192 { // 8GB threshold
...
pub fn update_active_transaction_count(delta: i32) {
    if delta > 0 {
        PERFORMANCE_METRICS.active_transactions.fetch_add(delta as usize, Ordering::Relaxed);
    } else if delta < 0 {
        PERFORMANCE_METRICS.active_transactions.fetch_sub((-delta) as usize, Ordering::Relaxed);
    }
}
...
pub fn reset_metrics() {
    ...
    PERFORMANCE_METRICS.total_inserts.store(0, Ordering::Relaxed);
    PERFORMANCE_METRICS.bulk_inserts.store(0, Ordering::Relaxed);
    PERFORMANCE_METRICS.buffer_hits.store(0, Ordering::Relaxed);
    PERFORMANCE_METRICS.buffer_misses.store(0, Ordering::Relaxed);
    PERFORMANCE_METRICS.disk_reads.store(0, Ordering::Relaxed);
    PERFORMANCE_METRICS.disk_writes.store(0, Ordering::Relaxed);
    PERFORMANCE_METRICS.lock_requests.store(0, Ordering::Relaxed);
    PERFORMANCE_METRICS.transactions_started.store(0, Ordering::Relaxed);
    PERFORMANCE_METRICS.transactions_committed.store(0, Ordering::Relaxed);
    PERFORMANCE_METRICS.transactions_aborted.store(0, Ordering::Relaxed);
    PERFORMANCE_METRICS.lock_conflicts.store(0, Ordering::Relaxed);
    PERFORMANCE_METRICS.version_chain_traversals.store(0, Ordering::Relaxed);
    PERFORMANCE_METRICS.select_operations.store(0, Ordering::Relaxed);
    PERFORMANCE_METRICS.update_operations.store(0, Ordering::Relaxed);
    PERFORMANCE_METRICS.delete_operations.store(0, Ordering::Relaxed);
