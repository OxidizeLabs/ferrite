...
impl Optimizer {
    ...
    pub fn optimize(
        ...
        // Final validation of the optimized plan
    ...
    fn apply_early_pruning(&self, mut plan: Box<LogicalPlan>) -> Result<Box<LogicalPlan>, DBError> {
        match &plan.plan_type {
            LogicalPlanType::Project {
                ...
                // Remove unnecessary projections (e.g., SELECT * directly from table)
            ...
            LogicalPlanType::Filter { predicate } => {
                // Simplify filter conditions (e.g., WHERE true)
    ...
    // Helper methods for child node processing
