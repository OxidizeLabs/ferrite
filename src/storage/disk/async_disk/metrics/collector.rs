...
impl MetricsCollector {
    ...
    pub fn record_read(&self, latency_ns: u64, bytes: u64, success: bool) {
        ...
        self.live_metrics.read_latency_sum.fetch_add(latency_ns, Ordering::Relaxed);
    ...
    pub fn record_write(&self, latency_ns: u64, bytes: u64) {
        ...
        self.live_metrics.write_latency_sum.fetch_add(latency_ns, Ordering::Relaxed);
        self.live_metrics.io_latency_sum.fetch_add(latency_ns, Ordering::Relaxed);
        self.live_metrics.io_count.fetch_add(1, Ordering::Relaxed);
        self.live_metrics.io_throughput_bytes.fetch_add(bytes, Ordering::Relaxed);
    ...
    pub fn record_batch_read(&self, operation_count: usize, total_latency_ns: u64, total_bytes: u64) {
        self.live_metrics.batch_ops_count.fetch_add(1, Ordering::Relaxed);
        self.live_metrics.read_ops_count.fetch_add(operation_count as u64, Ordering::Relaxed);
        self.live_metrics.read_latency_sum.fetch_add(total_latency_ns, Ordering::Relaxed);
        self.live_metrics.io_latency_sum.fetch_add(total_latency_ns, Ordering::Relaxed);
        self.live_metrics.io_count.fetch_add(operation_count as u64, Ordering::Relaxed);
        self.live_metrics.io_throughput_bytes.fetch_add(total_bytes, Ordering::Relaxed);
    }
    ...
    pub fn record_batch_write(&self, operation_count: usize, total_latency_ns: u64, total_bytes: u64) {
        self.live_metrics.batch_ops_count.fetch_add(1, Ordering::Relaxed);
        self.live_metrics.write_ops_count.fetch_add(operation_count as u64, Ordering::Relaxed);
        self.live_metrics.write_latency_sum.fetch_add(total_latency_ns, Ordering::Relaxed);
        self.live_metrics.io_latency_sum.fetch_add(total_latency_ns, Ordering::Relaxed);
        self.live_metrics.io_count.fetch_add(operation_count as u64, Ordering::Relaxed);
        self.live_metrics.io_throughput_bytes.fetch_add(total_bytes, Ordering::Relaxed);
        self.live_metrics.total_bytes_written.fetch_add(total_bytes, Ordering::Relaxed);
    }

    /// Records flush operation metrics
    pub fn record_flush(&self, latency_ns: u64, pages_flushed: usize) {
        self.live_metrics.flush_count.fetch_add(1, Ordering::Relaxed);
        self.live_metrics.io_latency_sum.fetch_add(latency_ns, Ordering::Relaxed);
        self.live_metrics.io_count.fetch_add(pages_flushed as u64, Ordering::Relaxed);
    ...
    pub fn update_performance_counters(&self) {
        ...
        if elapsed_secs > 0 {
            ...
            let total_bytes = self.live_metrics.io_throughput_bytes.load(Ordering::Relaxed);
            let bytes_per_sec = total_bytes / elapsed_secs;
            self.live_metrics.bytes_per_second.store(bytes_per_sec, Ordering::Relaxed);
            ...
            self.live_metrics.transactions_per_second.store(transactions_per_sec, Ordering::Relaxed);
            
            // Update uptime
            self.live_metrics.uptime_seconds.store(elapsed_secs, Ordering::Relaxed);
        }
    }
    ...
    pub fn reset_metrics(&self) {
        ...
        self.live_metrics.read_latency_sum.store(0, Ordering::Relaxed);
        self.live_metrics.write_latency_sum.store(0, Ordering::Relaxed);
        self.live_metrics.io_count.store(0, Ordering::Relaxed);
        self.live_metrics.io_throughput_bytes.store(0, Ordering::Relaxed);
        self.live_metrics.read_ops_count.store(0, Ordering::Relaxed);
        self.live_metrics.write_ops_count.store(0, Ordering::Relaxed);
        self.live_metrics.batch_ops_count.store(0, Ordering::Relaxed);
        self.live_metrics.flush_count.store(0, Ordering::Relaxed);
        self.live_metrics.total_bytes_written.store(0, Ordering::Relaxed);
    ...
    pub fn create_metrics_snapshot(&self) -> MetricsSnapshot {
        ...
        let io_throughput_mb_per_sec = if elapsed_secs > 0 {
            self.live_metrics.io_throughput_bytes.load(Ordering::Relaxed) as f64 / elapsed_secs_f64 / 1_000_000.0
        } else {
            0.0
        };
    ...
    pub fn create_snapshot_from_live(live_metrics: &LiveMetrics) -> MetricsSnapshot {
        ...
        let io_throughput_mb_per_sec = if elapsed_secs > 0 {
            live_metrics.io_throughput_bytes.load(Ordering::Relaxed) as f64 / elapsed_secs_f64 / 1_000_000.0
        } else {
            0.0
        };
...
mod tests {
    ...
    fn test_separate_read_write_latency_tracking() {
        ...
        let read_ops = collector.live_metrics.read_ops_count.load(Ordering::Relaxed);
        let write_ops = collector.live_metrics.write_ops_count.load(Ordering::Relaxed);
        ...
        let read_latency_sum = collector.live_metrics.read_latency_sum.load(Ordering::Relaxed);
        let write_latency_sum = collector.live_metrics.write_latency_sum.load(Ordering::Relaxed);
        assert_eq!(read_latency_sum, 6000, "Read latency sum should be 1000+2000+3000 = 6000");
        assert_eq!(write_latency_sum, 12000, "Write latency sum should be 5000+7000 = 12000");
        
        // Verify error counting
        let error_count = collector.live_metrics.error_count.load(Ordering::Relaxed);
        assert_eq!(error_count, 1, "Should have recorded 1 error from failed read");
        
        // Create snapshot and verify average latencies
        let snapshot = collector.create_metrics_snapshot();
        assert_eq!(snapshot.read_latency_avg_ns, 2000, "Average read latency should be 6000/3 = 2000ns");
        assert_eq!(snapshot.write_latency_avg_ns, 6000, "Average write latency should be 12000/2 = 6000ns");
    }
    ...
    fn test_batch_operation_metrics() {
        ...
        let read_ops = collector.live_metrics.read_ops_count.load(Ordering::Relaxed);
        let write_ops = collector.live_metrics.write_ops_count.load(Ordering::Relaxed);
        let batch_ops = collector.live_metrics.batch_ops_count.load(Ordering::Relaxed);
        
        assert_eq!(read_ops, 10, "Should have recorded 10 read operations from batch");
        assert_eq!(write_ops, 5, "Should have recorded 5 write operations from batch");
        ...
        assert_eq!(read_latency_sum, 50000, "Read latency sum should include batch latency");
        assert_eq!(write_latency_sum, 25000, "Write latency sum should include batch latency");
        
        // Verify bytes tracking
        let total_bytes_written = collector.live_metrics.total_bytes_written.load(Ordering::Relaxed);
        assert_eq!(total_bytes_written, 20480, "Should track bytes written from batch write");
    }
    ...
    fn test_cache_operation_metrics() {
        ...
        let hot_cache_hits = collector.live_metrics.hot_cache_hits.load(Ordering::Relaxed);
        let warm_cache_hits = collector.live_metrics.warm_cache_hits.load(Ordering::Relaxed);
        let promotions = collector.live_metrics.cache_promotions.load(Ordering::Relaxed);
        let demotions = collector.live_metrics.cache_demotions.load(Ordering::Relaxed);
        ...
        let snapshot = collector.create_metrics_snapshot();
        assert_eq!(snapshot.cache_hit_ratio, 0.75, "Cache hit ratio should be 3/4 = 0.75");
    }
    ...
    fn test_time_based_metrics_calculation() {
        ...
        let bytes_per_sec = collector.live_metrics.bytes_per_second.load(Ordering::Relaxed);
        let ops_per_sec = collector.live_metrics.pages_per_second.load(Ordering::Relaxed);
        let uptime = collector.live_metrics.uptime_seconds.load(Ordering::Relaxed);
        
        assert!(bytes_per_sec > 0, "Bytes per second should be calculated");
        assert!(ops_per_sec > 0, "Operations per second should be calculated");
    ...
    fn test_health_score_calculation() {
        ...
        assert!(health_score >= 90, "Health score should be high with reasonable latencies: {}", health_score);
        ...
        assert!(health_score_after < health_score, "Health score should decrease with high latencies");
        ...
        assert!(health_score_final < health_score_after, "Health score should decrease with errors");
    }
    ...
    fn test_metrics_reset() {
        ...
        assert!(collector.live_metrics.read_ops_count.load(Ordering::Relaxed) > 0);
        assert!(collector.live_metrics.write_ops_count.load(Ordering::Relaxed) > 0);
        ...
        assert_eq!(collector.live_metrics.read_ops_count.load(Ordering::Relaxed), 0);
        assert_eq!(collector.live_metrics.write_ops_count.load(Ordering::Relaxed), 0);
        assert_eq!(collector.live_metrics.flush_count.load(Ordering::Relaxed), 0);
        assert_eq!(collector.live_metrics.read_latency_sum.load(Ordering::Relaxed), 0);
        assert_eq!(collector.live_metrics.write_latency_sum.load(Ordering::Relaxed), 0);
        assert_eq!(collector.live_metrics.io_latency_sum.load(Ordering::Relaxed), 0);
    }
    ...
    fn test_metrics_snapshot_consistency() {
        ...
        assert!(snapshot.read_latency_avg_ns > 0, "Should have read latency data");
        assert!(snapshot.write_latency_avg_ns > 0, "Should have write latency data");
        assert!(snapshot.io_throughput_mb_per_sec >= 0.0, "Should have throughput data");
        ...
        assert_eq!(snapshot.write_latency_avg_ns, snapshot2.write_latency_avg_ns);
    ...
    fn test_zero_division_handling() {
        ...
        assert_eq!(snapshot.read_latency_avg_ns, 0, "Read latency should be 0 with no operations");
        assert_eq!(snapshot.write_latency_avg_ns, 0, "Write latency should be 0 with no operations");
        assert_eq!(snapshot.cache_hit_ratio, 0.0, "Cache hit ratio should be 0 with no cache operations");
        assert_eq!(snapshot.io_throughput_mb_per_sec, 0.0, "Throughput should be 0 with no operations");
    ...
    async fn test_monitoring_background_task() {
        ...
        assert!(handle.is_finished(), "Background task should be finished after abort");
    }
}