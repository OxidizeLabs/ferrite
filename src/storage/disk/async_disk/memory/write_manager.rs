...
impl WriteManager {
    ...
    pub async fn get_coalescing_size_analysis(&self, adjacent_pages: &[PageId], new_data: &[u8]) -> CoalescedSizeInfo {
    ...
    pub async fn update_coalescing_config(&self, coalesce_window: std::time::Duration, max_coalesce_size: usize) {
        let mut coalescing = self.coalescing_engine.write().await;
...
mod tests {
    ...
    async fn test_flush_coordination() {
        ...
        // Add some data
        write_manager.buffer_write(1, vec![1, 2, 3, 4]).await.unwrap();
        write_manager.buffer_write(2, vec![5, 6, 7, 8]).await.unwrap();
    ...
    async fn test_coalescing_integration() {
        ...
        // First write
        write_manager.buffer_write(page_id, first_data).await.unwrap();
        
        // Second write to same page (should be coalesced/merged)
        write_manager.buffer_write(page_id, second_data.clone()).await.unwrap();
    ...
    async fn test_coalescing_config_update() {
        ...
        write_manager.update_coalescing_config(
    ...
    async fn test_concurrent_flushes() {
        ...
        let flush1 = tokio::spawn(async move {
            wm1.flush().await
        });
        
        let flush2 = tokio::spawn(async move {
            wm2.flush().await
        });
