    ...
    fn split_child(&self, parent: &mut BPlusTreeNode, child_idx: usize) -> bool {
        ...
        match child_guard.node_type {
            NodeType::Leaf => {
                ...
                // For leaf nodes, keep keys sorted
                let mut entries: Vec<_> = child_guard
                    .keys
                    .iter()
                    .cloned()
                    .zip(child_guard.values.iter().cloned())
                    .collect();
                ...
                // Insert into parent maintaining sorted order
                let split_key = new_node_arc.read().keys[0].clone();
                let parent_pos = self.find_insert_position(&parent.keys, &split_key);
                parent.keys.insert(parent_pos, split_key);
                parent.children.insert(parent_pos + 1, new_node_arc.clone());
            ...
            NodeType::Internal => {
                // For internal nodes, maintain sorted order of keys
                let mut all_keys = child_guard.keys.clone();
                all_keys.sort_by(|key1, key2| {
                    if key1.compare_less_than(key2) == CmpBool::CmpTrue {
                        std::cmp::Ordering::Less
                    } else if key1.compare_greater_than(key2) == CmpBool::CmpTrue {
                        std::cmp::Ordering::Greater
                ...
                let split_key = all_keys[middle].clone();
                new_node.keys = all_keys[middle + 1..].to_vec();
                child_guard.keys = all_keys[..middle].to_vec();
                ...
                parent
                    .children
                    .insert(parent_pos + 1, Arc::new(RwLock::new(new_node)));
                return true;
            }
        }

        // For leaf nodes, insert into parent maintaining sorted order
...
impl Index for BPlusTree {
    ...
    fn scan_key(&self, key: &Tuple, result: &mut Vec<RID>, _transaction: &Transaction) {
        ...
        let binding = key.keys_from_tuple(key_attrs);
        let key = binding.get(0).unwrap();
            ...
            match node.node_type {
                NodeType::Internal => {
                    let pos = node
                        ...
                        .iter()
                        .position(|k| {
                            self.compare_keys_ordering(k, key) == std::cmp::Ordering::Greater
                        })
                        .unwrap_or(node.keys.len());
                    let next_node = Arc::clone(&node.children[pos]);
                    drop(node);
                ...
                NodeType::Leaf => {
                    ...
                    for (idx, k) in node.keys.iter().enumerate() {
                        if self.compare_keys_ordering(k, key) == std::cmp::Ordering::Equal {
...
mod basic_behavior_tests {
    ...
    fn test_basic_deletion() {
        ...
        tree_write_guard.scan_key(&tuple, &mut result, &Default::default());
