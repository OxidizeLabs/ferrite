...
mod unit_tests {
    ...
    fn basic_page_guard() {
        ...
        let basic_guard = BasicPageGuard::new(Arc::clone(&bpm), Arc::clone(&page0));
        ...
        drop(write_guard);
        assert_eq!(page0.read().get_pin_count(), 0, "Pin count should be 0 after dropping the WritePageGuard");
    }

    #[test]
    fn upgrade_to_read() {
        let ctx = TestContext::new("upgrade_to_read");
        let bpm = Arc::clone(&ctx.bpm);

        // Create a new guarded page
        let page = bpm.new_page().unwrap();
        let basic_guard = BasicPageGuard::new(Arc::clone(&bpm), Arc::clone(&page));

        // Assert that the page was pinned and the guard is valid
        assert_eq!(page.read().get_pin_count(), 1, "Pin count should be 1 after creating the BasicPageGuard");
        ...
        drop(read_guard);
        assert_eq!(page.read().get_pin_count(), 0, "Pin count should be 0 after dropping the ReadPageGuard");
    }

    #[test]
    fn upgrade_to_write() {
        let ctx = TestContext::new("upgrade_to_write");
        let bpm = Arc::clone(&ctx.bpm);
        ...
        drop(write_guard);
        assert_eq!(page.read().get_pin_count(), 0, "Pin count should be 0 after dropping the WritePageGuard");
    }
}
...
mod basic_behaviour {
    ...
    fn create_and_drop() {
        ...
        // Create a new guarded page with bpm
        let page0 = bpm.new_page().unwrap();
        let basic_guarded_page = BasicPageGuard::new(Arc::clone(&bpm), Arc::clone(&page0));
