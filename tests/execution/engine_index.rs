...
async fn create_index_composite_indexes() {
	...
	for sql in [
		...
		let ok = db.execute_sql(sql, IsolationLevel::ReadCommitted, &mut w).await.unwrap();
		assert!(ok);
	}
...
async fn create_index_error_cases() {
	...
	for sql in [
		...
		let res = db.execute_sql(sql, IsolationLevel::ReadCommitted, &mut w).await;
		assert!(res.is_err());
	}
...
async fn create_index_in_transaction() {
	...
	let ok = db.execute_sql("ROLLBACK", IsolationLevel::ReadCommitted, &mut w).await.unwrap();
	assert!(ok);
