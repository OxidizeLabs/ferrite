...
fn benchmark_constant_time(c: &mut Criterion) {
    ...
    for size in [100, 1_000, 10_000, 100_000] {
        ...
        group.bench_with_input(
            BenchmarkId::new("array_access", size),
            &size,
            |b, _| {
                b.iter(|| {
        ...
        group.bench_with_input(
            BenchmarkId::new("hash_lookup", size),
            &size,
            |b, _| {
                b.iter(|| {
...
fn benchmark_quadratic_time(c: &mut Criterion) {
    ...
    for size in [50, 100, 200, 400, 800] {
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    black_box(bubble_sort(data.clone()))
                })
            },
        );
...
fn benchmark_linearithmic_time(c: &mut Criterion) {
    ...
    for size in [100, 500, 1_000, 5_000, 10_000, 50_000] {
        ...
        group.bench_with_input(
            BenchmarkId::new("efficient_sort", size),
            &size,
            |b, _| {
                b.iter(|| {
                    black_box(efficient_sort(data.clone()))
                })
            },
        );
    }
...
fn benchmark_complexity_comparison(c: &mut Criterion) {
    ...
    group.bench_function("O(1)_array_access", |b| {
        b.iter(|| {
            black_box(constant_time_access(&data, size / 2))
        })
    });
    
    group.bench_function("O(1)_hash_lookup", |b| {
        b.iter(|| {
            black_box(hash_lookup(&map, (size / 2) as i32))
        })
    });
    ...
    group.bench_function("O(n)_sum_array", |b| {
        b.iter(|| {
            black_box(sum_array(&data))
        })
    });
