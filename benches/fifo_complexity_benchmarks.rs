...
fn benchmark_eviction_time_complexity(c: &mut Criterion) {
    ...
    for cache_size in [100, 500, 1000] {
        // Smaller sizes due to O(n) nature
        group.bench_with_input(
            ...
            |b, &cache_size| {
                b.iter_batched(
                    || {
                        ...
                        for i in 0..stale_count {
                            cache.insert(
                                format!("replace_key_{}", i),
                                format!("replace_value_{}", i),
                            );
                        }
                ...
                b.iter_batched(
                    || {
                        ...
                        for i in 0..cache_size {
                            cache.insert(
                                format!("key_{}", i),
                                format!("large_value_{}", "x".repeat(100)),
                            );
                        }
                        cache
                    },
...
fn benchmark_memory_pressure_scenarios(c: &mut Criterion) {
    ...
    for value_size in [100, 1000, 5000] {
        // bytes per value
        group.bench_with_input(
    ...
    group.bench_function("small_cache_mixed_workload", |b| {
        b.iter_batched(
            ...
            |mut cache| {
                ...
                for i in 0..10000 {
                    if i % 3 == 0 {
                        cache.insert(format!("key_{}", i % 200), format!("value_{}", i));
                    } else {
    ...
    group.bench_function("medium_cache_mixed_workload", |b| {
        b.iter_batched(
            ...
            |mut cache| {
                ...
                for i in 0..50000 {
                    if i % 5 < 2 {
                        cache.insert(format!("key_{}", i % 3000), format!("value_{}", i));
                    } else {
...
criterion_group!(fifo_operations_benches, benchmark_fifo_operations);
